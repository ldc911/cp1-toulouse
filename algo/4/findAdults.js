...
function findAdults(arr) {
  const myArray = [];
  const adultWomen = arr.filter(person => (person.age > 17 && person.sex == "female"));
  const adultMen = arr.filter(person => (person.age > 17 && person.sex == "male"));
  myArray.push(adultWomen);
  myArray.push(adultMen);
  return myArray;
}
